...
async def moderate_if_is_unsafe(bot,message):
    ...
    try:
        if  await heitanAPI.is_required_moderation(message.content):
            ...
            embed = discord.Embed(title="メッセージが修正されました",description=f"{message.author.display_name}@{message.author.name}", color=0xff0000)
            moderated_text = await heitanAPI.moderate(message.content)
            embed.add_field(name="修正前:", value=f"||{message.content}||", inline=False)
